...
export function StatBlock({ label, value, sub, tone = "default", spark }: StatBlockProps) {
  ...
  return (
    <div className="border-r border-b border-line bg-paper p-6 flex flex-col gap-2.5 min-h-[128px] relative overflow-hidden">
      <div className="eyebrow">{label}</div>
      <div className={`text-[34px] leading-[1] font-semibold tracking-tighter tabular-nums ${toneClass}`}>{value}</div>
